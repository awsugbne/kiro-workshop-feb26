...
2. **Review the requirements document**
   ...
   <Aside type="tip" title="What is EARS notation?">
   **EARS** (Easy Approach to Requirements Syntax) is a structured format for writing unambiguous requirements:

   - **Ubiquitous**: "The [system] shall [action]" — Always-on behaviour
   - **Event-driven**: "When [event], the [system] shall [action]" — Triggered behaviour
   - **State-driven**: "While [state], the [system] shall [action]" — Conditional behaviour
   - **Optional**: "Where [feature], the [system] shall [action]" — Feature-dependent behaviour
   - **Unwanted**: "If [condition], then the [system] shall [action]" — Exception handling

   This notation helps Kiro generate precise, testable requirements that capture edge cases automatically.
   </Aside>
